...
for(i in 1:N){
	Y.tilde[i,]=rmvnorm(1,mean=c(Mu1[i],Mu2[i]),sigma=matrix(c(1,rho.true*Dat[i,2],rho.true*Dat[i,2],1),2,2))
}
Dat[,3]=(Y.tilde[,1]>0)
Dat[,4]=(Y.tilde[,2]>0)
...
MCMC.cor[1]=0.5
MH.sig=0.05
Cor=MCMC.cor[1]*Dat[,2]

full_cond_cor<-function(Y.tilde,Mu,cor.par,Dist){ #on log scale!
	Delta=Y.tilde-Mu
	Cor=Dist*cor.par
	-.5*(sum(log(1-Cor^2))+sum((Delta[,1]^2-2*Cor*Delta[,1]*Delta[,2]+Delta[,2]^2)/(1-Cor^2)))
}
...
for(iiter in 2:n.iter){
	...
	X=rbind(X1-Cor*X2,X2-Cor*X1)
	V.inv <- crossprod(X,rep(Sig.inv,2)*X) 	
	Y=c(Y.tilde[,1]-Cor*Y.tilde[,2],Y.tilde[,2]-Cor*Y.tilde[,1])
	M.z <- solve(V.inv, crossprod(X,rep(Sig.inv,2)*Y))
	MCMC.beta[iiter,] <- M.z + solve(chol(V.inv), rnorm(n.beta,0,1))
	
	#update cor parameter  (metropolis-hastings)
	Mu1=X1%*%MCMC.beta[iiter,]
	Mu2=X2%*%MCMC.beta[iiter,]
	cor.star=MCMC.cor[iiter]+runif(1,-MH.sig,MH.sig)
	if(cor.star>0 & cor.star<1){		
		logP.old=full_cond_cor(Y.tilde,Mu=cbind(Mu1,Mu2),cor.par=MCMC.cor[iiter],Dist=Dat[,2])
		logP.new=full_cond_cor(Y.tilde,Mu=cbind(Mu1,Mu2),cor.par=cor.star,Dist=Dat[,2])
		if(runif(1)<exp(logP.new-logP.old)){
			MCMC.cor[iiter]=cor.star
			Cor=MCMC.cor[iiter]*Dat[,2]
		}
	}
