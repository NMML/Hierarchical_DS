...
run_spatial_sim=function(){
	...
	n.transects=n.real.transects*4 #each transect spans four cells
	Observers=matrix(NA,2,n.transects)
	Obs.cov=array(0,dim=c(2,n.transects,1))
	Obs.cov[1,,]=1
	n.obs.cov=dim(Obs.cov)[3]
	for(i in 1:n.real.transects){
		#Observers[,((i-1)*4+1):((i-1)*4+4)]=rep(sample(c(1,2,3),size=2,replace=FALSE),4)
		if(i%%2==1)Observers[,((i-1)*4+1):((i-1)*4+4)]=rep(sample(c(1,2,3),size=2,replace=FALSE),4)
		else Observers[1,((i-1)*4+1):((i-1)*4+4)]=rep(sample(c(1,2,3),size=1,replace=FALSE),4)
	}
	...
	Hab.cov=data.frame(matrix(rep(log(c(1:n.real.transects)/n.real.transects),n.real.transects),S,1))
	...
	#Cov.prior.parms=matrix(c(3,0,0,0,1,1,1,1),4,2)
	...
	#M=c(11:20)
	#M=c(21:50)
	Control=list(iter=75000,burnin=5000,thin=5,MH.cor=0.2,MH.nu=.01,MH.beta=c(.2,.4),RJ.N=rep(5,S),adapt=1000)
	Inits=NULL
