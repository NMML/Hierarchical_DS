...
mcmc_ds_misID<-function(Par,Data,cur.iter,adapt,Control,DM.hab,DM.det,Q,Prior.pars,Meta){	
	#require(mvtnorm)
	#require(Matrix)
	#require(truncnorm)
	...
	#initialize lambda
	Lambda=t(t(exp(Par$Nu))*Meta$Area.hab)
	Lambda.trans=t(t(Lambda[,Meta$Mapping])*Meta$Area.trans)
	...
	MCMC=list(N.tot=matrix(0,Meta$n.species,mcmc.length),N=array(0,dim=c(Meta$n.species,mcmc.length,Meta$S)),G=array(0,dim=c(Meta$n.species,mcmc.length,Meta$S)),Hab=array(0,dim=c(Meta$n.species,mcmc.length,ncol(Par$hab))),Det=data.frame(matrix(0,mcmc.length,length(Par$det))),cor=rep(0,mcmc.length),tau.eta=matrix(0,Meta$n.species,mcmc.length),tau.nu=matrix(0,Meta$n.species,mcmc.length),Cov.par=array(0,dim=c(Meta$n.species,mcmc.length,length(Par$Cov.par[1,,]))))
	#colnames(MCMC$Hab)=colnames(DM.hab)
	colnames(MCMC$Det)=colnames(DM.det)
	Accept=list(cor=0,N=matrix(0,Meta$n.species,Meta$n.transects),Nu=rep(0,n.species),Hab=matrix(0,Meta$n.species,max(Meta$N.hab.par)))
	...
	for(iiter in 1:cur.iter){
		...
		#1) for sampled cells
			DM=eval(parse(text=paste("DM.hab$sp",isp,sep='')))
			Hab=Par$hab[isp,1:Meta$N.hab.par[isp]]
			Eta=Par$Eta[isp,]
			Mu=DM%*%Hab+Eta
			G.sampled=rep(0,n.unique) #total number of groups currently in each sampled strata
			for(i in 1:Meta$n.transects)G.sampled[which(Sampled==Meta$Mapping[i])]=G.sampled[which(Sampled==Meta$Mapping[i])]+Meta$G.transect[isp,i]
			...
			new.post=log_lambda_log_likelihood(Log.lambda=Prop[Sampled],DM=DM,Beta=Hab,SD=sqrt(1/Par$tau.nu[isp]),N=G.sampled,Sampled=Sampled,Area=Sampled.area.by.strata)
			old.post=log_lambda_log_likelihood(Log.lambda=Par$Nu[isp,Sampled],DM=DM,Beta=Hab,SD=sqrt(1/Par$tau.nu[isp]),N=G.sampled,Sampled=Sampled,Area=Sampled.area.by.strata)
			...
			if(Meta$spat.ind==0){
				if(Meta$srr==FALSE){
					...
					M.eta <- solve(V.eta.inv, Par$tau.nu[isp]*(Par$Nu[isp,]-DM%*%Hab))		
					...
					#Update Theta
					Dat.minus.Exp=Par$Nu[isp,]-DM%*%Hab
					V.eta.inv <- cross.L*Par$tau.nu[isp] + Par$tau.eta[isp]*Qt
					M.eta <- solve(V.eta.inv, Par$tau.nu[isp]*L%*%Dat.minus.Exp)
					Theta <- M.eta + solve(chol(as.matrix(V.eta.inv)), rnorm(n.theta,0,1))
					Par$Eta[isp,]=as.numeric(L.t%*%Theta)
					
					#update tau.eta
					Par$tau.eta[isp] <- rgamma(1, n.theta/2 + Prior.pars$a.eta, as.numeric(crossprod(Theta, Qt %*% Theta)/2) + Prior.pars$b.eta)
				}
			}
		
			#update tau_nu	 (precision for Poisson overdispersion)
			Mu=DM%*%Hab+Par$Eta[isp,]
			...
			Mu.old=Mu
			for(ipar in 1:Meta$N.hab.par[isp]){
				Prop=Par$hab[isp,1:Meta$N.hab.par[isp]]
				Prop[ipar]=Prop[ipar]+Control$MH.beta[isp,ipar]*rnorm(1,0,1)
				old.post=sum(dnorm(Par$Nu[isp,],Mu.old,sqrt(1/Par$tau.nu[isp]),log=1))+dnorm(Par$hab[isp,ipar],0,Prior.pars$beta.sd,log=1)
				Mu=DM%*%Prop+Par$Eta[isp,]			
				new.post=sum(dnorm(Par$Nu[isp,],Mu,sqrt(1/Par$tau.nu[isp]),log=1))+dnorm(Prop[ipar],0,Prior.pars$beta.sd,log=1)
				if(runif(1)<exp(new.post-old.post)){
					Par$hab[isp,ipar]=Prop[ipar]
					Accept$Hab[isp,ipar]=Accept$Hab[isp,ipar]+1
					Mu.old=Mu
				}
			}		
			...
			for(itrans in 1:Meta$n.transects){
				...
				if(a>0){ # proposal an addition
					if(((Meta$G.transect[isp,itrans]+a)*Meta$n.Observers[itrans])>=Meta$M[isp,itrans]){
						...
						if(runif(1)<MH.prob){
							...
							else Meta$N.transect[isp,itrans]=sum(Data[isp,itrans,1:n.Records[isp,itrans],which(Meta$stacked.names=="Group")])/Meta$n.Observers[itrans]
				...
				if(Meta$n.ind.cov>0){
					for(icov in 1:Meta$n.ind.cov){
						...
						rsamp=switch_sample(n=(Meta$M[isp,itrans]-n.Records[isp,itrans])/Meta$n.Observers[itrans],pdf=Meta$Cov.prior.pdf[isp,icov],cur.par=Par$Cov.par[isp,,icov],RE=cur.RE)
				...
				if(cur.G>0){
					...
					if(Meta$n.ind.cov>0){
						for(icov in 1:Meta$n.ind.cov){
							...
							Cov.star=switch_sample(n=cur.G,pdf=Meta$Cov.prior.pdf[isp,icov],cur.par=Par$Cov.par[isp,,icov],RE=cur.RE)
							Cur.dat=Data[isp,itrans,(G.obs[isp,itrans]*Meta$n.Observers[itrans]+1):(Meta$G.transect[isp,itrans]*Meta$n.Observers[itrans]),]
							if(is.vector(Cur.dat))Cur.dat=matrix(Cur.dat,1,length(Cur.dat))
							cur.dist=Cur.dat[,Meta$dist.pl]
							X=get_mod_matrix(Cur.dat=Cur.dat,Meta$stacked.names,Meta$factor.ind,Meta$Det.formula,Meta$Levels)
				...
				for(isp in 1:Meta$n.species){
					...
					if(n.gt.one>1){
						for(itrans in 1:n.gt.one){
							...
							if(is.vector(Cur.dat))Cur.dat=matrix(Cur.dat,1,length(Cur.dat))
							Dist=c(Dist,matrix(Cur.dat[,Meta$dist.pl],Meta$G.transect[isp,I.gt.one[itrans]],Meta$n.Observers[I.gt.one[itrans]],byrow=TRUE)[,1])
			...
			for(icov in 1:Meta$n.ind.cov){
				if(Meta$Cov.prior.fixed[isp,icov]==0){
					...
					if(Meta$Cov.prior.pdf[isp,icov]=="multinom"){
						...
						Par$Cov.par[isp,,icov]=rdirichlet(1,Meta$Cov.prior.parms[isp,,icov]+tabulate(factor(Cur.cov)))
					}
				}
			}
		}
		...
		if(iiter>Control$burnin & iiter%%Control$thin==0){
			...
			for(isp in 1:Meta$n.species){
				...
				Temp.G=Meta$Area.hab[Mapping]*Meta$Area.trans*exp(rnorm(Meta$n.transects,(eval(parse(text=paste("DM.hab$sp",isp,sep='')))%*%Par$hab[isp,]+Par$Eta[isp,])[Mapping],sqrt(1/Par$tau.nu[isp])))
